...
module.exports = {
	allProductBrand: async (req, res) => {
		try {
			...
			const dataProductBrand = await productBrandModel.allProductBrandData(data);
			...
			if (search) {
				const pagination = {
					...
					dataPerPage: limitValue > dataProductBrand.rowCount ? dataProductBrand.rowCount : limitValue,
	...
	allProductBrandActive: async (req, res) => {
		try {
			...
			const dataProductBrandActive = await productBrandModel.allProductBrandActiveData(data);
			...
			if (search) {
				const pagination = {
					currentPage: pageValue,
					dataPerPage: limitValue > dataProductBrandActive.rowCount ? dataProductBrandActive.rowCount : limitValue,
	...
	addProductBrand: async (req, res) => {
		try {
			...
			const productNameCheck = await productBrandModel.productBrandNameCheck(brandName);
	...
	updateProductBrand: async (req, res) => {
		try {
			...
			const { brandName } = req.body;
			let photo;
			const productBrandDetail = await productBrandModel.detailProductBrandData(id);
