...
const productBrandModel = {
	productBrandNameCheck: (brandName) => {
		return new Promise((resolve, reject) => {
			db.query(`SELECT * FROM product_brands WHERE brand_name='${brandName}'`, (err, res) => {
				if (err) {
					reject(err);
				}
				resolve(res);
	...
	allProductBrandData: (data) => {
		return new Promise((resolve, reject) => {
			...
			db.query(`SELECT * FROM product_brands WHERE LOWER(brand_name) LIKE LOWER ('%${searchQuery}%') ORDER BY ${sortQuery} ${modeQuery} LIMIT ${limitValue} OFFSET ${offsetValue}`, (err, result) => {
				if (err) {
					reject(err);
				}
				resolve(result);
	...
	allProductBrandActive: () => {
		return new Promise((resolve, reject) => {
			db.query("SELECT COUNT(*) AS total from product_brands WHERE is_active=true", (err, res) => {
				if (err) {
					reject(err);
				}
				resolve(res);
	...
	statusProductBrandData: (data) => {
		return new Promise((resolve, reject) => {
			...
			db.query(`UPDATE product_brands SET is_active=${isActive} WHERE id='${id}'`, (err, res) => {
				if (err) {
					reject(err);
				}
				resolve(res);
