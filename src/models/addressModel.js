...
const addressModel = {
	...
	addAddressData: (data) => {
		return new Promise((resolve, reject) => {
			const { id, userId, label, recipientName, recipientPhone, address, postalCode, city, isPrimaryValue } = data;
			db.query(
				`
	...
	updateAddressData: (data) => {
		const { id, label, recipientName, recipientPhone, address, postalCode, city, isPrimaryValue } = data;
		return new Promise((resolve, reject) => {
			db.query(
				`
