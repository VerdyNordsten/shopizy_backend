...
const categoryModel = {
	...
	allCategoryActive: () => {
		return new Promise((resolve, reject) => {
			db.query("SELECT COUNT(*) AS total from categories WHERE is_active=true", (err, res) => {
				if (err) {
					reject(err);
				}
				resolve(res);
	...
	allCategoryData: (data) => {
		return new Promise((resolve, reject) => {
			const { searchQuery, offsetValue, limitValue, sortQuery, modeQuery } = data;
			db.query(`SELECT * FROM categories WHERE LOWER(category_name) LIKE LOWER ('%${searchQuery}%') ORDER BY ${sortQuery} ${modeQuery} LIMIT ${limitValue} OFFSET ${offsetValue}`, (err, result) => {
				if (err) {
					reject(err);
				}
				resolve(result);
	...
	categoryNameCheck: (categoryName) => {
		return new Promise((resolve, reject) => {
			db.query(`SELECT * FROM categories WHERE category_name='${categoryName}'`, (err, result) => {
				if (err) {
					reject(err);
				}
				resolve(result);
	...
	updateCategoryData: (data) => {
		return new Promise((resolve, reject) => {
			...
			db.query(`UPDATE categories SET category_name='${categoryName}', photo='${photo}' WHERE id='${id}'`, (err, result) => {
				if (err) {
					reject(err);
				}
				resolve(result);
	...
	statusCategoryData: (data) => {
		return new Promise((resolve, reject) => {
			...
			db.query(`UPDATE categories SET is_active=${isActive} WHERE id='${id}'`, (err, res) => {
				if (err) {
					reject(err);
				}
				resolve(res);
